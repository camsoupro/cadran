...
export const STR = {
  ...
  pPrices: ["Prix et coûts", "Prices and costs"],
  pPlanner: ["Échéancier", "Schedule"],
  pCampaigns: ["Campagnes", "Campaigns"],
  navSteer: ["Piloter", "Steer"],

  // ---------------------------------------------------------------- prix et couts
  prTitleBook: ["Base de prix et coûts de revient", "Price book and cost prices"],
  prReadBook: [
    "Ce que paie l'entreprise, poste par poste, et ce que coûte réellement chaque produit une fois la nomenclature déroulée. <b>Un entrepreneur doit pouvoir refaire ce calcul seul</b>, sans attendre son comptable, parce que c'est lui qui fixe les prix. Le simulateur plus bas permet de changer une hypothèse et de voir la marge bouger.",
    "What the business pays, line by line, and what each product really costs once the bill of materials is unrolled. <b>An owner must be able to redo this on their own</b>, without waiting for the accountant, because they are the one setting prices. The simulator below lets you change an assumption and watch the margin move."],
  prMaterials: ["Matières premières", "Raw materials"],
  prPackaging: ["Emballages", "Packaging"],
  prServices: ["Services et main d'oeuvre", "Services and labour"],
  prOverheads: ["Frais fixes mensuels", "Monthly fixed costs"],
  prRef: ["Réf.", "Ref."],
  prDesignation: ["Désignation", "Description"],
  prUnit: ["Unité", "Unit"],
  prUnitPrice: ["Prix unitaire", "Unit price"],
  prSupplier: ["Fournisseur", "Supplier"],
  prProducts: ["Coût de revient, produit par produit", "Cost price, product by product"],
  prCost: ["Coût de revient", "Cost price"],
  prSellPrice: ["Prix de vente HT", "Selling price"],
  prMargin: ["Marge", "Margin"],
  prMarginPct: ["Taux de marge", "Margin rate"],
  prCoef: ["Coefficient", "Multiplier"],
  prQty: ["Quantité", "Quantity"],
  prAmount: ["Montant", "Amount"],
  prSim: ["Simulateur de marge", "Margin simulator"],
  prSimRead: [
    "Changez une hypothèse et regardez la marge bouger. Ce sont les trois leviers que vous tenez vraiment : le prix auquel vous achetez, le rendement de votre torréfaction, et le prix auquel vous vendez.",
    "Change an assumption and watch the margin move. These are the three levers you actually hold: what you pay, the yield you get, and what you charge."],
  prSimGreen: ["Prix du café vert, le kilo", "Green coffee price per kilo"],
  prSimYield: ["Rendement de torréfaction", "Roasting yield"],
  prSimPrice: ["Prix de vente HT", "Selling price"],
  prSimProduct: ["Produit", "Product"],
  prSimResult: ["Coût recalculé", "Recomputed cost"],
  prSimBreak: ["Prix plancher, marge nulle", "Floor price, zero margin"],
  prSimWarn: [
    "À ce prix de vente, chaque unité vendue perd de l'argent.",
    "At this selling price, every unit sold loses money."],
  prSimOk: [
    "Il reste {margin} par unité, soit {pct} du prix de vente.",
    "{margin} is left on each unit, that is {pct} of the selling price."],

  // ---------------------------------------------------------------- echeancier
  plTitle: ["Échéancier et rappels", "Schedule and reminders"],
  plRead: [
    "Tout ce qui tombe, dans un seul calendrier : les encaissements attendus, les factures fournisseurs à régler, la TVA, la paie, les cotisations, et les dossiers de suivi à relancer. Les montants sont signés : ce qui entre en vert, ce qui sort en rouge.",
    "Everything that falls due, in one calendar: expected receipts, supplier invoices to pay, VAT, payroll, contributions, and the follow ups waiting on you. Amounts are signed: money in is green, money out is red."],
  plLate: ["En retard", "Overdue"],
  plSoon: ["Sous 30 jours", "Within 30 days"],
  plImpact: ["Effet net sur la trésorerie", "Net effect on cash"],
  plKind: ["Nature", "Kind"],
  plWhat: ["Échéance", "Item"],
  plKClient: ["Client", "Customer"],
  plKFournisseur: ["Fournisseur", "Supplier"],
  plKFiscal: ["Fiscal", "Tax"],
  plKPaie: ["Paie", "Payroll"],
  plKSocial: ["Social", "Contributions"],
  plKSuivi: ["Suivi", "Follow up"],
  plNothing: ["Rien à cette échéance.", "Nothing due here."],
  plReminder: ["{n} rappels", "{n} reminders"],
  plWhy: ["À quoi sert un échéancier", "What a schedule is for"],
  plWhyText: [
    "Un compte de résultat dit si l'année a été bonne. Un échéancier dit si le mois prochain est tenable. Les deux sont nécessaires, et c'est le second qui fait fermer les entreprises quand il manque : sur les trente prochains jours, l'effet net attendu est de <b>{impact}</b>.",
    "An income statement says whether the year was good. A schedule says whether next month is survivable. Both are needed, and it is the second one whose absence closes businesses: over the next thirty days the expected net effect is <b>{impact}</b>."],

  // ---------------------------------------------------------------- campagnes
  cpTitle: ["Campagnes publicitaires", "Advertising campaigns"],
  cpRead: [
    "Cinq campagnes depuis janvier, classées par ce qu'elles rapportent pour un euro dépensé. La dépense totale correspond au compte 623, publicité, du compte de résultat. <b>Le coût par commande ne suffit pas à juger</b> : la campagne la plus chère par commande est ici la plus rentable, parce qu'elle gagne des clients professionnels qui recommandent chaque semaine.",
    "Five campaigns since January, ranked by what they return for a euro spent. Total spend matches account 623, advertising, in the income statement. <b>Cost per order is not enough to judge</b>: the most expensive campaign per order is the most profitable one here, because it wins trade customers who reorder every week."],
  cpChannel: ["Canal", "Channel"],
  cpPeriod: ["Période", "Period"],
  cpBudget: ["Budget", "Budget"],
  cpSpent: ["Dépensé", "Spent"],
  cpOrders: ["Commandes", "Orders"],
  cpPerOrder: ["Coût par commande", "Cost per order"],
  cpRevenue: ["Chiffre d'affaires attribué", "Revenue attributed"],
  cpReturn: ["Retour par euro", "Return per euro"],
  cpBest: ["La meilleure et la pire", "The best and the worst"],
  cpBestText: [
    "<b>{best}</b> rapporte {bestReturn} € pour un euro dépensé, alors qu'elle coûte {bestCost} par commande : ses commandes sont des professionnels qui reviennent. <b>{worst}</b> rapporte {worstReturn} € par euro : elle coûte plus qu'elle ne rapporte, et c'est la seule de la liste dans ce cas.",
    "<b>{best}</b> returns {bestReturn} EUR for every euro spent even though it costs {bestCost} per order: its orders are trade customers who come back. <b>{worst}</b> returns {worstReturn} EUR per euro: it costs more than it brings in, and it is the only one on the list in that position."],
